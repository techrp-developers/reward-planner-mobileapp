...
function getSelectedCartItemInput(source = {}) {
  const keys = ["selected_items", "selected_cart_item_ids", "cart_item_ids"];
  const key = keys.find((name) => Object.prototype.hasOwnProperty.call(source, name));

  return {
    hasInput: Boolean(key),
    value: key ? source[key] : undefined,
  };
}
...
class ServiceCheckoutController {
  ...
  async addToCheckout(req, res) {
    ...
    try {
      ...
      const selectedCartItems = getSelectedCartItemInput(req.body);
      const selectedCartItemIds = parseSelectedItemIds(selectedCartItems.value);
      ...
      if (selectedCartItems.hasInput && !selectedCartItemIds.length) {
        return res.status(400).json({
          success: false,
      ...
      const cartData = await CartModel.getCart(cart.id, selectedCartItemIds);
      ...
      if (!bundles.length && !individual_items.length) {
        return res.status(400).json({
          success: false,
          message: selectedCartItemIds.length
      ...
      if (selectedCartItemIds.length) {
        await CartModel.clearItems(cart.id, selectedCartItemIds, conn);
      } else {
  ...
  async getCheckoutPreview(req, res) {
    try {
      const userId = req.user?.user_id;
      const selectedCartItems = getSelectedCartItemInput(req.query);
      const selectedCartItemIds = parseSelectedItemIds(selectedCartItems.value);
