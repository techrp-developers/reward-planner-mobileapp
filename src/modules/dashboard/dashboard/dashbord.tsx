...
function Dashbord() {
  ...
  const topSectionGradient: string[] = isFestive
    ? isDark
      ? ['#09090B', '#14532D', '#1F2937']
      : ['#FFF7ED', '#FFE8CC', '#FFEDD5']
    : isDark
      ? ['#09090B', '#111827', '#18181B']
      : ['#111827', '#1E1B4B', '#312E81'];

  const rootGradient = isFestive
    ? isDark
      ? ['#09090B', '#111827', '#0F2A18']
      : ['#FFF7ED', '#F0FDF4', '#FFFFFF']
    : isDark
      ? ['#09090B', '#111827', '#151526']
      : ['#F8FAFC', '#EEF2FF', '#FFFFFF'];

  const quoteBannerGradient: string[] = isFestive
    ? isDark
      ? ['#7C2D12', '#1F2937', '#14532D']
      : ['#FF9933', '#FDBA74', '#138808']
    : isDark
      ? ['#18181B', '#27233A', '#4338CA']
      : ['#111827', '#312E81', '#4F46E5'];

  // Android needs an explicit barStyle/backgroundColor — it doesn't infer
  // contrast from the gradient the way iOS effectively does.
  const statusBarDark = isFestive && !isDark;
  ...
  return (
    ...
    >
      <StatusBar
        ...
        backgroundColor={Platform.OS === 'android' ? topSectionGradient[0] : undefined}
      />
      <ScrollView
        style={styles.scroll}
