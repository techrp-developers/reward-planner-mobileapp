...
export default function Health() {
  ...
  const [resultCoverAmount, setResultCoverAmount] = useState(500000);
  const [resultEnquiryId, setResultEnquiryId] = useState<number | undefined>(
    undefined
  );
  ...
  const ensureEnquiryId = async () => {
    ...
    try {
      const startRes = await startInsurance("health");
      const enquiryId = startRes?.enquiry_id || startRes?.enquiryId || startRes?.id;

      if (!enquiryId) {
        throw new Error("Unable to parse enquiry ID from response");
      }
  ...
  const handleShowResults = (results: any) => {
    ...
    success("Quotes Fetched!", `Found ${successCount} customized plans for you`);
  };

  const handleBackToForm = () => {
    setShowResults(false);
    setStep(2);
    info("Go Back", "You can modify your details and fetch new quotes");
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: servicesTheme.colors.background }]}>
      ...
      >
        ...
        {step === 0 && <Step1 data={formData} setData={setFormData} onNext={handleNextStep} />}
