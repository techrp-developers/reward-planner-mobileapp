...
class ServiceCartModel {
  ...
  // get cart items
  async getCart(cartId, selectedItemIds = []) {
    const selectedIds = [...new Set(selectedItemIds)]
      .map((id) => Number(id))
      .filter((id) => Number.isInteger(id) && id > 0);
    const selectedItemFilter = selectedIds.length
      ? ` AND ci.id IN (${selectedIds.map(() => "?").join(", ")})`
      : "";
  ...
  async clearItems(cartId, itemIds, conn = db) {
    ...
    await conn.execute(
      `DELETE FROM service_cart_items
       WHERE cart_id = ? AND id IN (${selectedIds.map(() => "?").join(", ")})`,
      [cartId, ...selectedIds],
    );
  }
